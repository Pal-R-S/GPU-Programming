...
void make_kernel_call(Func kernel_name, unsigned int N, Args... args) {
...
template <typename Func, typename... Args>
void time_kernel_call(Func kernel_name, unsigned int N, Args... args) {
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    cudaEventRecord(start);
    
    make_kernel_call(kernel_name, N, args...);
